...
AmqpStream.prototype.receive = function (callback) {
  var stream = this.stream;
  stream.connect(this.event, function () {
    stream.on('message', function (msg, next) {
      callback({body: JSON.parse(msg.content.toString()), properties: msg.properties, fields: msg.fields, amqpMsg: msg}, next);
    });
  });
};
